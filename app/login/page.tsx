    ...
    <div className="bg-jules-bg text-jules-primary font-mono antialiased h-screen flex flex-col relative overflow-hidden">
      ...
      <nav className="absolute top-0 w-full p-6 flex justify-between items-center z-10">
        <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
          <div className="w-6 h-6 rounded bg-jules-text flex items-center justify-center text-jules-bg font-bold text-xs font-mono">&lt;/&gt;</div>
      ...
      <main className="flex-1 flex items-center justify-center p-4 z-10">
        <div className="w-full max-w-[420px] animate-slide-up">
          <div className="bg-jules-surface border border-jules-border rounded-3xl p-8 shadow-2xl relative overflow-hidden">

            <div className="text-center mb-8">
              <h1 className="text-2xl font-semibold mb-2">Welcome back</h1>
              <p className="text-sm text-jules-muted">Enter your details to access your workspace.</p>
            </div>

            <div className="grid grid-cols-2 gap-3 mb-6">
              <button className="flex items-center justify-center gap-2 bg-[#252526] hover:bg-[#2f2f30] border border-jules-border text-white py-2.5 rounded-full text-sm font-medium transition-all hover:border-jules-muted/50">
                <svg className="w-4 h-4" viewBox="0 0 24 24"><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" style={{ fill: '#4285F4' }}></path><path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" style={{ fill: '#34A853' }}></path><path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" style={{ fill: '#FBBC05' }}></path><path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" style={{ fill: '#EA4335' }}></path></svg>
                Google
              </button>
              <button className="flex items-center justify-center gap-2 bg-[#252526] hover:bg-[#2f2f30] border border-jules-border text-white py-2.5 rounded-full text-sm font-medium transition-all hover:border-jules-muted/50">
            ...
            <div className="relative flex py-2 items-center mb-6">
              <div className="flex-grow border-t border-jules-border"></div>
              <span className="flex-shrink-0 mx-4 text-xs text-jules-muted uppercase tracking-wider">Or email</span>
              <div className="flex-grow border-t border-jules-border"></div>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
              {error && <div className="text-jules-error text-sm text-center">{error}</div>}

              <div className="relative group">
                <input
                  type="email"
                  id="email"
                  className="peer w-full bg-transparent border border-jules-border rounded-xl px-4 py-3 text-sm text-white placeholder-transparent focus:outline-none focus:border-jules-accent focus:ring-1 focus:ring-jules-accent transition-all"
              ...
              <div className="flex justify-end">
                <a href="#" className="text-xs text-jules-accent hover:underline">Forgot password?</a>
              </div>
            ...
            <div className="mt-6 text-center text-sm text-jules-muted">
              <span>Don't have an account?</span>
              <Link href="/register" className="text-jules-accent font-medium hover:underline ml-1">Create account</Link>
            </div>

          </div>

          <div className="mt-8 flex justify-center gap-6 text-xs text-jules-muted">
